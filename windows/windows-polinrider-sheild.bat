...
echo.
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "$tc=[System.Environment]::ExpandEnvironmentVariables('%TCFILE%'); $jobs=Get-PSDrive -PSProvider FileSystem | ForEach-Object { $d=$_.Root; Start-Job -ScriptBlock { param($r) $out=@(); Get-ChildItem $r -Recurse -Force -ErrorAction SilentlyContinue | Where-Object { $_.Name -in 'tasks.json','settings.json' -and $_.FullName -match '\.vscode' -and $_.FullName -notmatch 'polinrider-shield' } | ForEach-Object { $c=Get-Content $_.FullName -Raw -ErrorAction SilentlyContinue; if($c -match 'folderOpen'){ if($c -match 'woff2|woff|ttf|fonts'){ $out+='RED::[MALICIOUS TASK - folderOpen+font] '+$_.FullName } else { $out+='YEL::[REVIEW TASK - folderOpen] '+$_.FullName } } }; return $out } -ArgumentList $d }; $count=0; $jobs | Wait-Job | ForEach-Object { $o=Receive-Job $_; if($o){ $o | ForEach-Object { if($_ -match '^RED::'){ Write-Host ($_ -replace '^RED::','') -ForegroundColor Red; $count++ } else { Write-Host ($_ -replace '^YEL::','') -ForegroundColor Yellow } } } }; $jobs | Remove-Job; $c=[int](Get-Content $tc -ErrorAction SilentlyContinue); Set-Content $tc ($c+$count)"
